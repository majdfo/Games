        ...
        for i in range(cell[0]-1,cell[0]+2):
            for j in range (cell[1]-1,cell[1]+2):
                ...
                if 0<=i <self.height and 0<=j<self.width and (i,j) not in self.safes and (i,j)not in self.mines:
                    undetermincell.append((i,j))
        newsentence=Sentence(undetermincell,count-countmines)
        ...
        for i in range(self.height):
            for j in range (self.width):
                if(i,j) not in self.moves_made and (i,j) not in self.mines:
                    possiplemove.append((i,j))
        if len(possiplemove) !=0:
            return random.choice(possiplemove)
        else:
            return None
